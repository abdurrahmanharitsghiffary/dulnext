...
def update_comment_meta(doctype: str, name: str):
    ...
    filters = [["reference_name", "=", name], ["comment_type", "=", "Comment"], ["reference_doctype", "=", doctype]]

    comments = frappe.db.get_list("Comment", fields=["content", "comment_by", "name"], filters=filters)

    print(f"Comments: {comments}")

    cached_comments = [{"comment": comment.get("content", ""), "by": comment.get("comment_by"), "name": comment.get("name")} for comment in comments]

    redis_doc_meta.update_meta(doctype, name, "_comments", json.dumps(cached_comments))


@frappe.whitelist(methods=["POST", "PUT"])
def add_comment(reference_doctype: str, reference_name: str, content: str, comment_email: str, comment_by: str) -> "Comment":
